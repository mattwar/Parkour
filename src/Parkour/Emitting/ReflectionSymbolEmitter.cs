...
public class ReflectionSymbolEmitter : SymbolEmitter
{
    ...
    public ReflectionSymbolEmitter(
    ...
    public override void DefineType(TypeSymbol typeSymbol)
    {
        var name = typeSymbol.FullName;
        TypeBuilder typeBuilder = _moduleBuilder.DefineType(name, GetTypeAttributes(typeSymbol));
    ...
    public override void DefineMember(MemberSymbol memberSymbol)
    {
        if (memberSymbol.DeclaringSymbol is TypeSymbol declaringTypeSymbol
            && _symbolToBuilder.TryGetValue(memberSymbol.DeclaringSymbol, out var builder)
            && builder is TypeBuilder typeBuilder)
        {
            switch (memberSymbol)
            {
                ...
                case ConstructorSymbol constructor:
                    ...
                    _symbolToBuilder.Add(constructor, constructorBuilder);
        ...
        else if (memberSymbol.DeclaringSymbol is NamespaceSymbol ns)
        {
            switch (memberSymbol)
            {
                // global method
        ...
        if (_symbolToBuilder.TryGetValue(symbol, out var builder))
        {
            switch (builder)
            {
                case MethodBuilder methodBuilder:
                    fnEmitBody(symbol, new ReflectionILEmitter(this, methodBuilder.GetILGenerator()));
                    break;
                case ConstructorBuilder constructorBuilder:
                    fnEmitBody(symbol, new ReflectionILEmitter(this, constructorBuilder.GetILGenerator()));
                    break;
            }
        }       
    }

    public override void EmitDefinedTypesAndMembers()
    {
    ...
    private class ReflectionILEmitter : ILEmitter
    {
        ...
        public ReflectionILEmitter(ReflectionSymbolEmitter emitter, ILGenerator ilgen)
        {
            _emitter = emitter;
            _ilgen = ilgen;
        }
